...
for(const p of ps){
 ...
 function boundary(axis,fraction){const size=axis==='x'?w:h;let best=Math.round(size*fraction),score=0;for(let n=Math.round(size*(fraction-.045));n<size*(fraction+.045);n++){let s=0;const length=axis==='x'?h:w;for(let q=0;q<length;q++)if(white(axis==='x'?n:q,axis==='x'?q:n))s++;if(s>score){score=s;best=n;}}return score/(axis==='x'?h:w)>.85?best:Math.round(size*fraction);}

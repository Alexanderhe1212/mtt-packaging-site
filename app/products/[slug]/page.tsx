...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}){const p=productBySlug((await params).slug);return p?{title:`Custom ${p.name} | ${p.code} | MTT Packaging`,description:p.description,alternates:{canonical:`/products/${p.slug}`,languages:{en:`/products/${p.slug}`,'zh-Hans':`/zh/products/${p.slug}`}},openGraph:{images:[p.image.replace(".webp","-0.webp")]}}:{}}

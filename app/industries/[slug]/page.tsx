...
export default async function IndustryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="industry-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(data) }} />
    ...
    <header><div><p>{item.eyebrow} packaging</p><h1>{item.title}</h1><p>{item.summary}</p><div className="tag-row">{item.products.map((p) => <span key={p}>{p}</span>)}</div></div><img src={item.image} alt={item.imageAlt} width="900" height="900"/></header>

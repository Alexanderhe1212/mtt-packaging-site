...
export default function CaseStudiesPage() {
  return <main id="main-content"><SiteNav />
    ...
    <header className="case-heading"><p className="section-kicker">Case studies</p><h1>Explore our packaging case directory.</h1><p>Explore 10 packaging studies: two anonymized project discussions and eight independent design analyses based on credited industry references. Each study explains product requirements, structure, materials and the complete packaging presentation.</p></header>
    <section className="case-directory" aria-label="Packaging case directory">{cases.map((a,i)=><article key={a.slug}>
      <a href={`/insights/${a.slug}`} aria-label={`Read case study: ${a.title}`}><img src={caseStages[a.slug]?.image || a.image} alt={`Four-stage packaging visualization: ${a.title}`} width="1600" height="1600" loading="lazy" /></a>
      <div className="case-copy"><p className="section-kicker">{String(i+1).padStart(2,'0')} · {a.angle}</p><h2><a href={`/insights/${a.slug}`}>{a.title}</a></h2><p>{a.summary}</p><p className="case-stage">{a.angle === 'Design Analysis' ? 'Independent design analysis · Sources included' : 'Anonymized project discussion'}</p><a className="case-link" href={`/insights/${a.slug}`}>Explore the four stages →</a></div>
    </article>)}</section>

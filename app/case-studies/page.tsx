...
export default function CaseStudiesPage() {
  return <main id="main-content"><SiteNav />
    ...
    <section className="case-directory" aria-label="Packaging case directory">{cases.map((a,i)=><article key={a.slug}>
      ...
      <div className="case-copy"><p className="section-kicker">{String(i+1).padStart(2,'0')} · {a.angle}</p><h2><a href={`/insights/${a.slug}`}>{a.title}</a></h2><p>{a.summary}</p><p className="case-stage">{a.angle === 'Design Analysis' ? 'Independent design analysis · Sources included' : 'Anonymized project discussion'} · Concept illustration</p><a className="case-link" href={`/insights/${a.slug}`}>Explore this case →</a></div>
    </article>)}</section>
